from notears.ensemble_locally_connected import EnsembleLayer, EnsembleLocallyConnected
...
class EnsembleNotearsMLP(nn.Module):
    def __init__(self, dims, bias=True, ensemble_size=7):
        super(EnsembleNotearsMLP, self).__init__()
        assert len(dims) >= 2
        assert dims[-1] == 1
        d = dims[0]
        self.dims = dims
        ...
        self.fc1_pos = EnsembleLayer(d, d * dims[1], ensemble_size=ensemble_size, bias=bias)
        self.fc1_neg = EnsembleLayer(d, d * dims[1], ensemble_size=ensemble_size, bias=bias)
        ...
        for l in range(len(dims) - 2):
            layers.append(EnsembleLocallyConnected(d, dims[l + 1], dims[l + 2], ensemble_size=ensemble_size, bias=bias))
...
def ensemble_notears_mlp(model: nn.Module,
                         X: np.ndarray,
                         lambda1: float = 0.,
                         lambda2: float = 0.,
                         max_iter: int = 100,
...
def main():
    ...
    model = EnsembleNotearsMLP(dims=[d, 10, 1], ensemble_size=ensemble_size, bias=True)
